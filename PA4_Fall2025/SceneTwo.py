...
class SceneTwo(Scene):
    ...
    def __init__(self, shaderProg):
        ...
        ellipsoid = Component(Point((0.75, 0, 0.75)), DisplayableEllipsoid(shaderProg, color=ColorType.SOFTGREEN))
        ...
        l0 = Light(l0_pos, np.array([*ColorType.YELLOW, 1.0]), spotDirection=np.array((0.,-1.,0.)), spotAngleLimit=30*np.pi/180)
        # l0 = Light(l0_pos, np.array([0, 0, 0, 1.0]), spotDirection=np.array((0.,-1.,0.)), spotAngleLimit=30*np.pi/180)
        lightCube0 = Component(Point(l0_pos), DisplayableCube(shaderProg, 0.1, 0.1, 0.1, ColorType.YELLOW))
        lightCube0.renderingRouting = "vertex"

        l1_pos = np.array([-2., 0., 0.])
        l1 = Light(l1_pos, np.array([*ColorType.RED, 1.0]))
        ...
        l2 = Light(l2_pos, np.array([*ColorType.BLUE, 1.0]))
    ...
    def animationUpdate(self):
        for c in self.shapes:
            c.uAngle += 1
            c.uAngle %= 360
            c.rotate(c.uAngle, c.uAxis)
        self.party += 1
        if self.party == 50:
            self.toggleLight(random.randint(0, 2))
            self.party = 0
        pass
