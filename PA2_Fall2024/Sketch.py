...
class Sketch(CanvasBase):
    ...
    def InitGL(self):
        ...
        self.cDict['hat'].rotate(45, self.cDict['hat'].vAxis)
    ...
    def Interrupt_Scroll(self, wheelRotation):
        ...
        if len(self.components) > 0 and self.select_obj_index >= 0:
            if self.multiSelect:
                for index in self.multi_select_obj_index:
                    self.components[index].rotate(wheelChange * self.MOUSE_SCROLL_SPEED,
    ...
    def Interrupt_Keyboard(self, keycode):
        ...
        if keycode in [wx.WXK_RETURN]:
            ...
            if len(self.components) > 0:
                ...
                if self.multiSelect:
                    self.multi_select_obj_index.append(self.select_obj_index)
        ...
        if chr(keycode) in 'A':
            ...
            for index in self.multi_select_obj_index:
                self.components[index].reset("color")
            if self.multiSelect and self.select_obj_index >= 0:
                self.multi_select_obj_index = [self.select_obj_index]
            else:
                self.multi_select_obj_index = []
        ...
        if keycode in [wx.WXK_LEFT]:
            ...
            if self.select_obj_index >= 0:
                if self.multiSelect:
                    for index in self.multi_select_obj_index:
                        self.components[index].setCurrentColor(self.select_color[self.select_axis_index])
                else:
                    self.components[self.select_obj_index].setCurrentColor(self.select_color[self.select_axis_index])
            self.update()
        ...
        if chr(keycode) in "5":
            ...
            self.cDict['leftArm1'].rotate(0, self.cDict['leftArm1'].uAxis)
